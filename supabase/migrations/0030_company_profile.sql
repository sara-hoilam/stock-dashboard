-- Ticker Alpha — FMP company profile for the price-panel flip card
--
-- Apply after 0029. Safe to re-run.
--
-- The company page's metrics card flips to show who the company is: CEO, IPO
-- date, employees, sector/industry, country, exchange, and a short description.
-- All of that rides on the same FMP /profile call we already make for sector
-- and industry; we just keep the rest of the payload now.

alter table ledger.price_daily add column if not exists profile jsonb;
...
drop function if exists public.upsert_company_extras(text, jsonb, text, text, jsonb);
...
begin
  ...
  execute 'revoke all on function public.upsert_company_extras(text, jsonb, text, text, jsonb, jsonb) from public, anon, authenticated';
  execute 'grant execute on function public.upsert_company_extras(text, jsonb, text, text, jsonb, jsonb) to service_role';
end $$;
