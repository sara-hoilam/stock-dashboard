-- Ticker Alpha — company nickname search (SpaceX, Google, Facebook, …)
--
-- Apply after 0051. Safe to re-run.
--
-- Searching "spacex" only hit SpaceX *ETFs* because the equity's legal name is
-- "SPACE EXPLORATION TECHNOLOGIES CORP" (no "SpaceX" substring) and ticker
-- SPCX does not prefix-match SPACEX. Same gap for Google→GOOGL, Facebook→META,
-- BRK.B vs BRK-B, "JP Morgan", "Johnson and Johnson", etc.
--
-- Add a company_alias map (boosted like crypto/index aliases), include alias
-- hits even when the legal name does not contain the query, normalize ticker
-- punctuation (BRK.B ↔ BRK-B), and compact-match names so spaced queries still
-- hit concatenated legal names.
...
as $$
  ...
  alias_hits as (
    select coalesce(t.ticker, m.symbol) as ticker,
           ...
           t.cik,
           case
             when t.ticker is not null then 'stock'::text
             else coalesce(m.kind, 'stock'::text)
           end as kind,
           0 as kind_rank
